...
const LIQUIDITY_PREFERENCES: Record<string, string> = {
 ...
 */
export function recommendDEX(swapParams: SwapParams): DEXComparisonResult {
 ...
 */
function generateRecommendationReasoning(swapParams: SwapParams, recommendedDEX: DEXOption): string {
    const { fromAsset, toAsset, amount } = swapParams;
    const swapValueUSD = amount * getAssetPrice(fromAsset);
 ...
 */
export function compareDEXOptions(swapParams: SwapParams): {
    comparison: Record<string, DEXAnalysis>;
    summary: string;
} {
    const { fromAsset, toAsset, amount } = swapParams;
    const comparison: Record<string, DEXAnalysis> = {};
 ...
 */
function analyzeDEXForSwap(dex: DEXOption, swapParams: SwapParams): DEXAnalysis {
    const { fromAsset, toAsset, amount } = swapParams;
    const swapValueUSD = amount * getAssetPrice(fromAsset);
    ...
    const strengths: string[] = [...dex.advantages];
    const concerns: string[] = [...dex.tradeoffs];
 ...
 */
export function generateDEXLink(dexId: string, swapParams: SwapParams): string {
 ...
 */
export function generateDEXButtons(swapParams: SwapParams): string {
 ...
 */
export function getDEXStatistics(dexId: string): {
    volume24h: number;
    ...
    return mockStats[dexId as keyof typeof mockStats] || {
        volume24h: 0,
        totalLiquidity: 0,
 ...
 */
export function trackDEXSelection(dexId: string, swapParams: SwapParams, wasRecommended: boolean): void {
 ...
 */
export function getSupportedPairs(dexId: string): string[] {
    // Mock data - in production, fetch from DEX APIs
    ...
    return supportedPairs[dexId as keyof typeof supportedPairs] || [];
}

/**
 * Checks if a DEX supports a specific trading pair
 */
export function supportsPair(dexId: string, fromAsset: string, toAsset: string): boolean {
